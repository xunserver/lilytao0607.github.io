 ...
 * @param locale - The locale ('nl', 'en', 'de', 'pl', 'fr', 'zh')
...
export function parsePath(fullPath: string): { basePath: string; pathWithoutLocale: string } {
  ...
  const localeMatch = pathWithoutBase.match(/^\/(nl|en|de|pl|fr|zh)(\/|$)/);
  if (localeMatch) {
    return {
      basePath: base,
      pathWithoutLocale: pathWithoutBase.replace(/^\/(nl|en|de|pl|fr|zh)/, '') || '/'
    };
  }
